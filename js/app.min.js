...
},{"./properties":6,"./states/boot.js":7,"./states/play.js":8,"./states/preloader.js":9,"lodash":1}],4:[function(require,module,exports){
"use strict";
...
Shadow.create = function (game, args) {
    return new Shadow(game, args);
};

Shadow.prototype = {
    attachTo: function attachTo(actor) {
        this.actor = actor;
    ...
    update: function update() {
        ...
        var gap = Math.abs(this.ground - this.actor.body.position.y);
        var factor = 1 - gap / this.max_height;
...
},{"../../../package.json":2}],7:[function(require,module,exports){
'use strict';
...
},{"../../lib/stats.min":10,"../properties":6}],8:[function(require,module,exports){
'use strict';

var _ = require('lodash');
var properties = require('../properties');
var Bouncer = require('../objects/Bouncer');
var Shadow = require('../objects/Shadow');
...
function makeSplash(game, disc) {
    ...
    var splash = game.add.sprite(disc.body.position.x + 24, disc.bottom, 'splash', 5);
...
function ComboMachine(game, combos) {
    var _this = this;

    this.combos = combos;
    // Capture keys sequences
    game.input.keyboard.addCallbacks(this, null, // onDown
    null, // onUp
    function (UNUSED, key) {
        if (key.keyCode === Phaser.Keyboard.SPACEBAR) {
            ...
            _this.resetCount = 10;
            _this.sequence.push(key.key);
        }
    });
    this.reset();
}

ComboMachine.prototype = {
    reset: function reset() {
        this.resetCount = 10;
        this.sequence = [];
    },
    update: function update() {
        var combo = this.combos[this.sequence.join('|')];
        if (combo != null) {
            combo();
            this.reset();
        } else {
...
play.create = function create() {
    ...
    // NOTE Disc has to be created after shadow (or help find a way to play with the z-indexes ^.^)
    var shadow = createShadow(this.game);
    ...
    this.comboMachine = new ComboMachine(this.game, {
        'ArrowUp|ArrowUp|ArrowRight': function ArrowUpArrowUpArrowRight() {
            var vx = 750,
                time = 0.2;

            _this2.accelerateVx = _this2.disc.body.velocity.x;
            _this2.disc.body.velocity.x += vx;

            _this2.accelerateGoal = _this2.disc.body.position.x + vx * time;
        },

        'ArrowUp|ArrowUp|ArrowLeft': function ArrowUpArrowUpArrowLeft() {
            var pos = _this2.disc.body.position;
            _this2.blast = _this2.game.add.graphics(pos.x, pos.y);
            _this2.blast.alpha = 0.95;
            _this2.blast.beginFill(0x9b59b6);
            _this2.blast.drawCircle(2, 0, 16);
            _this2.blast.endFill();
        }
    });
};
...
play.update = function update() {
    ...
    if (this.accelerateVx != null && disc.body.position.x >= this.accelerateGoal) {
        disc.body.velocity.x = this.accelerateVx;
        delete this.accelerateVx;
        delete this.accelerateGoal;
    }

    if (this.blast != null) {
        this.blast.position.x = disc.body.position.x;
        this.blast.position.y = disc.body.position.y;
        this.blast.scale.x *= 1.07;
        this.blast.scale.y *= 1.07;
        this.blast.alpha *= 0.97;
        if (this.blast.scale.x >= 3) {
            this.blast.destroy();
            delete this.blast;
        }
    }
    ...
    if (disc.alive) {
        ...
        if (!disc.alive) {
            ...
            setTimeout(function () {
                _this3.game.state.clearCurrentState();
                _this3.game.state.restart();
            }, 750);
        }
    }
};
...
},{"../objects/Bouncer":4,"../objects/Shadow":5,"../properties":6,"lodash":1}],9:[function(require,module,exports){
'use strict';
